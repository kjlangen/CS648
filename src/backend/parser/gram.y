...
/* CS448 Added HASH and MERGE to the following list of keyword tokens */
...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	MAPPING MATCH MATERIALIZED MAXVALUE MERGE MINUTE_P MINVALUE MODE MONTH_P MOVE
 ...
 */
%left		JOIN CROSS HASH MERGE LEFT FULL RIGHT INNER_P NATURAL
...
joined_table:
			...
			| table_ref HASH JOIN table_ref join_qual
				{
					/*
					 * CS448 - added grammar for HASH join, and modified
					 * the parameters of the JoinExpr node to reflect
					 * the changes
					 */
					JoinExpr *n = makeNode(JoinExpr);
					n->jointype = JOIN_INNER;
					/* CS448 set isForcedJoin to TRUE */
					n->isForcedJoin = TRUE;
					/* CS448 set forcedJoinAlgorithm to HASH_JOIN */
					n->forcedJoinAlgorithm = HASH_JOIN;
					n->isNatural = FALSE;
					n->larg = $1;
					n->rarg = $4;
					n->usingClause = NIL;
			...
			| table_ref MERGE JOIN table_ref join_qual
				{
					/* CS448 - similar to the rule above, but for MERGE */
					JoinExpr *n = makeNode(JoinExpr);
					n->jointype = JOIN_INNER;
					/* CS448 set isForcedJoin to TRUE */
					n->isForcedJoin = TRUE;
					/* CS448 set forcedJoinAlgorithm to MERGE_JOIN */
					n->forcedJoinAlgorithm = MERGE_JOIN;
					n->isNatural = FALSE;
					n->larg = $1;
					n->rarg = $4;
					n->usingClause = NIL;
