...
_outJoinExpr(StringInfo str, const JoinExpr *node)
{
	...
	WRITE_NODE_FIELD(alias);
	WRITE_INT_FIELD(rtindex);
	/* CS448 - write hint fields in JoinExpr */
	WRITE_BOOL_FIELD(isForcedJoin);
	WRITE_ENUM_FIELD(forcedJoinAlgorithm, ForcedJoinAlgorithm);
}

/* CS448 - output a ForcedJoin object */
static void
_outForcedJoin(StringInfo str, const ForcedJoin *node)
{
	WRITE_NODE_TYPE("FORCEDJOIN");

	WRITE_ENUM_FIELD(join_algorithm, ForcedJoinAlgorithm);
}
	...
	else
	{
		...
		switch (nodeTag(obj))
		{
			...
			case T_JoinExpr:
				...
				break;
			/* CS448 - added a condition for ForcedJoin object */
			case T_ForcedJoin:
				_outForcedJoin(str, obj);
				break;
			case T_FromExpr:
